...
if (!commands.includes(arg)) {
  shell.echo(
    `Please supply one of the following command to run: ${commands.join(", ")}`,
  );
  shell.exit(1);
}
...
const runCommands = {
    win32: {
        ...
        link: () => shell.exec('mklink /D "%SPLUNK_HOME%\\etc\\apps\\badrcm" "%cd%\\stage"'),
    },
    nix: {
        build: () => shell.exec('export NODE_ENV=production && ./node_modules/.bin/webpack --mode=production'),
        link: () => shell.exec('ln -s $PWD/stage $SPLUNK_HOME/etc/apps/badrcm'),
    },
};
